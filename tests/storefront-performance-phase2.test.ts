...
test("home and products render server cards through small client interaction shells", () => {
  ...
  const homeHighlightPath = new URL("../components/home-highlight-product-card.tsx", import.meta.url)
  ...
  assert.equal(existsSync(homeHighlightPath), true)
  assert.equal(existsSync(productsBrowserPath), true)

  const carousel = readFileSync(homeCarouselPath, "utf8")
  const highlight = readFileSync(homeHighlightPath, "utf8")
