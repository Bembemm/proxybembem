...
test("cart stores only a short-lived CEP and selected freight preview before authenticated checkout", async () => {
  ...
  assert.match(
    cart,
    /catch\s*\{[\s\S]*\}[\s\S]*window\.location\.assign\(["']\/checkout["']\)/,
  )

  assert.ok(checkout.includes("readCheckoutPreview()"))
  assert.match(
    checkout,
    /function readCheckoutPreview\(\)[\s\S]*try\s*\{[\s\S]*window\.localStorage\.getItem/,
  )
  ...
  assert.match(
    checkout,
    /try\s*\{[\s\S]*localStorage\.removeItem\(CHECKOUT_PREVIEW_KEY\)[\s\S]*\}\s*catch[\s\S]*window\.location\.assign\(result\.checkoutUrl\)/,
  )
