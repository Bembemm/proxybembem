...
test("home renders the approved responsive multi-card highlights carousel", () => {
  ...
  assert.match(home, /data-carousel-item/)
  assert.ok(home.includes("w-[82vw]"))
  assert.ok(home.includes("sm:w-[calc((100%-1rem)/2)]"))
  assert.ok(home.includes("lg:w-[calc((100%-2rem)/3)]"))
  assert.ok(home.includes("xl:w-[calc((100%-3rem)/4)]"))
