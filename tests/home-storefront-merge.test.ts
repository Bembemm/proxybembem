...
test("home carousel keeps each product card together with reference arrows pagination and cart action", () => {
  ...
  assert.match(home, /className="group[^\"]*w-\[calc\(100vw-2rem\)\][^\"]*shrink-0[^\"]*snap-start/)
})

test("home and FAQ keep a compact vertical rhythm without forcing empty viewport height", () => {
  const home = source("components/pages/home-page.tsx")
  const faq = source("components/faq-section.tsx")

  assert.doesNotMatch(home, /min-h-\[70vh\]/)
  assert.match(home, /pb-8/)
  assert.match(home, /pt-24/)
  assert.match(home, /md:pt-32/)
  assert.match(faq, /pb-12/)
  assert.match(faq, /pt-8/)
  assert.match(faq, /sm:pt-10/)
})
