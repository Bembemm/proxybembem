...
export function ProductDetailModal({ product, isOpen, onClose }: ProductDetailModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95vw] max-h-[90vh] p-0 bg-slate-900/95 backdrop-blur-xl border border-[#8B5CF6]/30 overflow-hidden">
        ...
        <div className="p-5 md:p-6 flex flex-col">
          {/* Product Tag */}
          {product.tag && (
            <span className="self-start bg-[#8B5CF6] text-white text-xs font-semibold px-3 py-1 tracking-wider uppercase rounded mb-3">
              {product.tag}
            </span>
          )}
            ...
            <div className="leading-relaxed space-y-3">
              ...
              <div className="space-y-2">
                <p>
                  ...
                  <span className="text-sm text-gray-300">Este produto são PROXIES (cartas para teste/casual). NÃO SÃO CARTAS ORIGINAIS.</span>
                </p>
                
                <p>
                  <span className="font-bold text-white">VERSO:</span>{" "}
                  <span className="text-sm text-gray-300">BRANCO (Não tem arte no verso).</span>
                </p>
                
                <p>
                  <span className="font-bold text-white">SLEEVES:</span>{" "}
                  <span className="text-sm text-gray-300">NÃO INCLUSOS. É necessário o uso de sleeves (protetores) com fundo colorido/opaco para jogar.</span>
                </p>
                
                <p>
                  <span className="font-bold text-white">ARTES:</span>{" "}
                  <span className="text-sm text-gray-300">Trabalhamos com artes oficiais, alternativas do nosso banco de dados e também com Artes Personalizadas (fotos pessoais, logos ou desenhos próprios). Atenção: A confecção/impressão de artes próprias possui um custo adicional. Consulte os valores ao enviar sua lista no WhatsApp.</span>
                </p>
              </div>
              
              {/* Cabeçalho de Seção */}
              <p className="mt-4 font-bold text-white">O QUE ESTÁ INCLUSO</p>
              ...
              {/* Cabeçalho de Seção */}
              <p className="mt-4 font-bold text-white">COMO ENVIAR SUA LISTA</p>
              <div className="text-sm text-gray-300 space-y-1">
              ...
              {/* Cabeçalho de Seção */}
              <p className="mt-4 font-bold text-white">QUALIDADE PROXYBEMBEM</p>
          ...
          <Button 
            ...
            size="lg"
            className={`w-full py-6 text-base font-semibold tracking-wide transition-all duration-300 ${
              justAdded 
