...
function AddToCartButton({ product }: { product: Product }) {
  ...
  return (
    <Button 
      onClick={handleClick}
      size="sm" 
      className={`w-full tracking-wide text-xs sm:text-sm py-2.5 transition-all duration-300 ${
        justAdded 
      ...
      <section className="relative flex items-center justify-center overflow-hidden pt-16 sm:pt-20 pb-6 sm:pb-8">
        <div className="relative container mx-auto px-3 sm:px-4 py-4 sm:py-6 text-center z-10">
          <div className="bg-white/60 backdrop-blur-md border border-white/50 shadow-lg p-4 sm:p-6 md:p-8 max-w-2xl mx-auto rounded-lg">
            ...
            <p className="text-sm sm:text-base md:text-lg text-slate-700 max-w-xl mx-auto leading-relaxed text-pretty px-2">
      ...
      <section className="relative py-6 sm:py-8">
        <div className="container mx-auto px-3 sm:px-4 relative z-10">
          <div className="text-center mb-6 sm:mb-10 bg-white/60 backdrop-blur-md border border-white/50 shadow-lg p-4 sm:p-6 max-w-xl mx-auto rounded-lg">
            ...
            <p className="text-slate-700 text-xs sm:text-sm px-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto">
            {featuredProducts.map((product) => (
              <article 
                key={product.id} 
                className="group relative bg-white/60 backdrop-blur-md border border-white/50 shadow-lg overflow-hidden rounded-lg hover:shadow-xl hover:border-[#8B5CF6]/50 transition-all duration-300"
              >
                <div 
                ...
                >
                  ...
                  {product.tag && (
                    <span className="absolute top-2 left-2 bg-[#8B5CF6] text-white text-[10px] sm:text-xs font-semibold px-2 py-1 tracking-wider uppercase rounded">
                      {product.tag}
                    </span>
                  )}
                </div>
                ...
                <div className="p-3 sm:p-4 relative">
                  <h3 className="font-semibold text-slate-900 text-sm sm:text-base mb-2 line-clamp-2 min-h-[2.5rem] leading-snug">
                    {product.title}
                  </h3>
                  
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-xs text-slate-500 line-through opacity-70">
                      {formatPrice(product.originalPrice)}
                    </span>
                    <span className="text-base sm:text-lg font-bold text-[#8B5CF6]">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Button 
                      ...
                      variant="ghost"
                      size="sm" 
                      className="w-full text-xs py-1.5 text-slate-500 hover:text-[#8B5CF6] hover:bg-[#8B5CF6]/5"
                    >
          ...
          <div className="text-center mt-6 sm:mt-8">
            <Button 
              ...
              className="w-full sm:w-auto bg-transparent border border-[#8B5CF6] text-[#8B5CF6] px-6 sm:px-8 py-3 tracking-wide hover:bg-[#8B5CF6] hover:text-white hover:border-[#8B5CF6] transition-colors duration-300 text-sm sm:text-base"
            >
