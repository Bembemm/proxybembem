...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (userError || !data.user?.email_confirmed_at) {
      return json(401, { ok: false, message: "Link de recuperação inválido ou expirado." })
    }

    const { error: updateError } = await supabase.auth.updateUser({ password: input.password })
    if (updateError) {
      return json(400, { ok: false, message: "Não foi possível redefinir a senha." })
    }
    ...
    if (signOutError) {
      return json(503, {
        ok: false,
        message: "Senha redefinida, mas não foi possível encerrar todas as sessões. Entre novamente.",
      })
    }
  } catch {
    return json(503, { ok: false, message: "Serviço temporariamente indisponível." })
