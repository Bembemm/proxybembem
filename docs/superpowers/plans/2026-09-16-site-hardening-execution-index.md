...
Each plan must finish its focused tests before the next plan starts. The final plan reruns the complete regression/Vercel acceptance gate.
...
- Proxy-aware IP validation and Vercel trust boundary: plan 2.
...
If `RATE_LIMIT_TRUSTED_PROXY_HOPS` is not verified for Vercel, keep forwarding headers untrusted (`0`) and do not promote the candidate. Do not guess a production hop count.
...
The branch is merge-ready only when the final SHA passes `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm test`, the explicit commerce/security subset, Supabase Advisor review, and the isolated Vercel sandbox smoke described in plan 4. A green branch is not the same as a production deployment.